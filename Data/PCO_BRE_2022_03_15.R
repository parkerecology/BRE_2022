...
y_obs <- read_csv("BRE_2022_wide_Y_obsCovs_12vis.csv")
...
umf_bre <- unmarkedFramePCO(
  ...
  siteCovs = sc,
  obsCovs = list(
    J_date = as.data.frame(y_obs[, 266:529]),
    Season = as.data.frame(y_obs[, 530:793])),
  yearlySiteCovs = list(
...
save.image("BRE_2022_03_07.RData")
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
# previous year's drought conditions during july g ART ~95 mins (no SE)

tmp <- coef(l.HOR_g)
inits <- c(tmp[1:3], 0,tmp[4:8])

system.time(g.pdsi_july_p2_g <-
              pcountOpen(
                lambda = ~HOR,
                gamma  = ~pdsi_july_p + I(pdsi_july_p^2),
                omega  = ~1,
                p      = ~Season,
                umf_bre,
                K = kmax,
...
# previous year's cumulative SPEI drought conditions to July quadratic ART~135
...
# previous 2 year's cumulative SPEI drought conditions to july ART ~100 mins
...
# previous year's cumulative SPEI drought conditions to July quadratic ART ART ~135mins
...
# Current year's max temps during aug gompertz
system.time(g.tmax_july_c_g <-
              pcountOpen(
                lambda = ~HOR,
                gamma  = ~tmax_july_c,
...
summary(g.tmin_dec_p_g)
beep(8)

# previous year's min temps during jan exp
system.time(g.tmin_jan_p_g <- 
              pcountOpen(
                lambda = ~HOR, 
                gamma  = ~tmin_jan_p, 
                omega  = ~1,
...
# ~~~ save the work so far ~~~~~
save.image("BRE_2022_03_10.RData")
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
#pdsi previous year july and HOR gompertz 
system.time(go.pdsi_july_p_HOR_g <-
              pcountOpen(
                lambda = ~HOR,
                gamma  = ~pdsi_july_p,
                omega  = ~HOR,
...
summary(go.pdsi_july_p_HOR_g)
beep(1)



# #pdsi previous year^2 july and HOR gompertz 
# system.time(go.pdsi_july_p2_HOR_g <-
#               pcountOpen(
#                 lambda = ~HOR,
#                 gamma  = ~pdsi_july_p + I(pdsi_july_p^2),
#                 omega  = ~HOR,
...
#pdsi previous year july and DuPont gompertz 
system.time(go.pdsi_july_p_DP_g <-
              pcountOpen(
                lambda = ~HOR,
                gamma  = ~pdsi_july_p,
                omega  = ~DuPont,
...
# rank by AICc too
cand.set <- c(
  "lam(.)gam(.)omg(.)p(.)_g"                                      = m.null_g,
  "lam(.)gam(.)omg(.)p(.)_exp"                                    = m.null_exp,
  "lam(.)gam(.)omg(.)p(Season)"                                   = p.season,
  #"lam(HOR)gam(.)omg(.)p(Season)_r"                               = l.HOR_r,
  "lam(HOR)gam(.)omg(.)p(Season)_g"                               = l.HOR_g,
  ...
  #"lam(HOR)gam(PDSI July y-1)omg(.)p(Season)_r"                   = g.pdsi_july_p_r,
  "lam(HOR)gam(PDSI July y-1 ^2)omg(.)p(Season)_g"                = g.pdsi_july_p2_g,
  #"lam(HOR)gam(PDSI August y-1)omg(.)p(Season)_r"                 = g.pdsi_aug_p_r,
  "lam(HOR)gam(SPEI July T-180d)omg(.)p(Season)_g"                = g.spei180d_g,
  "lam(HOR)gam(SPEI July T-180d^2)omg(.)p(Season)_g"              = g.spei180d2_g,
  ...
  #"lam(HOR)gam(tmax July y-1)omg(.)p(Season)_r"                   = g.tmax_july_p_r,
  #"lam(HOR)gam(tmax July)omg(.)p(Season)_r"                       = g.tmax_july_c_r,
  #"lam(HOR)gam(tmax aug y-1)omg(.)p(Season)_exp"                  = g.tmax_aug_p_r,
  ...
  "lam(HOR)gam(pdsi july y-1 + tmin dec y-1)omg(.)p(Season)_exp"  = g.tmin_jan_pdsi_july_p_exp,
  "lam(HOR)gam(pdsi july y-1 )omg(HOR)p(Season)_g"                =go.pdsi_july_p_HOR_g,
  "lam(HOR)gam(pdsi july y-1 )omg(DuPont)p(Season)_g"           =go.pdsi_july_p_DP_g
)
...
ep <- predict(g.spei1y2_g, type = "gamma", newdata = nd, appendData = TRUE)
